...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public ArrayList<StickerInfo> getStickerInfoList(String category) {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM STICKERS_INFO WHERE MAIN_CATEGORY='" + category + "' AND " + IS_DOWNLOADED + " = '" + String.valueOf(true) + "'" + " ORDER BY " + SEQUENCE + " ASC", null);
        if (cursor == null || cursor.getCount() <= 0 || !cursor.moveToFirst()) {
            db.close();
    ...
    public ArrayList<StickerInfo> getStickerInfoList1(String category, int seq) {
        ArrayList<StickerInfo> componentInfoList = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM STICKERS_INFO WHERE IS_UPDATED='" + String.valueOf(true) + "' AND " + MAIN_CATEGORY + "='" + category + "' AND " + SEQUENCE + " > '" + String.valueOf(seq) + "'" + " ORDER BY " + SEQUENCE + " ASC;", null);
        if (cursor == null || cursor.getCount() <= 0 || !cursor.moveToFirst()) {
            db.close();
            return componentInfoList;
        }
    ...
    public ArrayList<StickerInfo> getStickerInfoByName(String stickerName) {
        ArrayList<StickerInfo> componentInfoList = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM STICKERS_INFO WHERE STICKER_NAME='" + stickerName + "'", null);
        if (cursor == null || cursor.getCount() <= 0 || !cursor.moveToFirst()) {
            db.close();
            return componentInfoList;
        }
